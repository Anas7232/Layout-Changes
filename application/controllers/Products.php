...
class Products extends CI_Controller
{
    ...
    {    	
        ...
        $attribute_names = explode('|', $attribute_names);                $filter['attribute_value_ids'] = get_attribute_ids_by_value($attribute_values, $attribute_names);
    ...
    public function category($category_slug = '')
    {
        ...
        $this->data['filters'] = get_attribute_values_by_id(array('1','2','3','4','5','6')); 
    ...
    public function check_zipcode1()
    {
        $this->form_validation->set_rules('product_id', 'Product Id', 'trim|numeric|xss_clean|required');
    ...
    }    //     public function getAllSubCategories($x=null) {    //     	$this->load->helper(['url', 'language', 'timezone_helper']);    //     	$this->load->model(['cart_model', 'category_model', 'rating_model']);    //     	$this->load->library(['pagination']);    //     	$this->data['settings'] = get_settings('system_settings', true);    //     	$this->data['web_settings'] = get_settings('web_settings', true);    //     	$this->data['is_logged_in'] = ($this->ion_auth->logged_in()) ? 1 : 0;    //     	$this->data['user'] = ($this->ion_auth->logged_in()) ? $this->ion_auth->user()->row() : array();    //     	//     	$this->response['csrfName'] = $this->security->get_csrf_token_name();    //     	//     	$this->response['csrfHash'] = $this->security->get_csrf_hash();    //     	$data = $_REQUEST ['head'];    //     	if(isset($data['slug'])){    //     	}//     	$category_id = get_category_id_by_slug($data['slug']);//     	$category = $this->category_model->get_categories($category_id, $limit = null, $offset = null, $sort = null, $order = null, $has_child_or_item = 'true');//     	print_r('<pre>');//     	print_r($category);//     	exit();//     	//     	$arrRes ['subCat'] = $this->category_model->fetchAllSubCatgories ( $category_id );    //     	echo json_encode($this->response);//     	//return false;//     }  
}
