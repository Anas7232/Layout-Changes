...
class Payment extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data['web_settings'] = get_settings('web_settings', true);                $this->data['facebookbtn'] = facebook_login();        $this->data['googlebtn'] = google_login();   
    }
    ...
    }        public function initiate_paytabs_transaction()        {    	$this->load->model ( 'User_model', 'user');    	    	if ($this->data['is_logged_in']) {        		$_POST['user_id'] = $this->data['user']->id;    		    		$cart = get_cart_total($this->data['user']->id, false, '0', $_POST['address_id']);               		$wallet_balance = fetch_details('id=' . $this->data['user']->id, 'users', 'balance');        		$wallet_balance = $wallet_balance[0]['balance'];        		$overall_amount = $cart['overall_amount'];        		if ($_POST['wallet_used'] == 1 && $wallet_balance > 0) {        			$overall_amount = $overall_amount - $wallet_balance;        		}        		if (!empty($_POST['promo_code'])) {        			$validate = validate_promo_code($_POST['promo_code'], $this->data['user']->id, $cart['total_arr']);        			if ($validate['error']) {        				$this->response['error'] = true;        				$this->response['message'] = $validate['message'];        				print_r(json_encode($this->response));        				return false;        			} else {        				$overall_amount = $overall_amount - $validate['data'][0]['final_discount'];        			}        		}        		$amount = $overall_amount;        		$user_id = $this->data['user']->id;    		    		$userdetails = fetch_users($user_id);    		    		$userAddress = $this->user->getSpecificUserAddressDetail($_POST['address_id']);        		$settings = get_settings('system_settings', true);        		$app_name = isset($settings['app_name']) && !empty($settings['app_name']) ? $settings['app_name'] : 'eShop - ecommerce';        		$app_name = str_replace(" ", "", $settings['app_name']);        		$order_id = $app_name . "-" . time() . rand(1000, 9999);        		$address = '';    		    		$request_data=  array(    				    				"profile_id"=> 68586,    				"tran_type"=> "sale",    				"tran_class"=> "ecom" ,    				"cart_id"=> $order_id,    				"cart_description"=> $order_id,    				"cart_currency"=> "PKR",    				"cart_amount"=> $amount,    				"callback"=>"https://eshop.samisalon.com/callback",    				"return"=> "https://eshop.samisalon.com/payment/paytabs_response/".$user_id,    				"hide_shipping"=>true,    				"customer_details" => array(    						"name"=> $userdetails[0]['username'],    						"email"=> $userdetails[0]['email'],    						    						"phone"=>$userAddress['mobile'],    						"street1"=> $userAddress['area'],    						"city"=> $userAddress['city'],    						"state"=> $userAddress['state'],    						"code"=> $userAddress['pincode'],    						"country"=> "PK",    						"ip"=> $_SERVER['REMOTE_ADDR']    				)    		);    	    		$curl = curl_init();    		curl_setopt_array($curl, array(    				CURLOPT_URL => 'https://secure-global.paytabs.com/payment/request',    				CURLOPT_RETURNTRANSFER => true,    				CURLOPT_ENCODING => '',    				CURLOPT_MAXREDIRS => 10,    				CURLOPT_TIMEOUT => 0,    				CURLOPT_CUSTOMREQUEST => 'POST',    				CURLOPT_POSTFIELDS => json_encode($request_data, true),    				CURLOPT_HTTPHEADER => array(    						'authorization:' . 'S2JNRRBHZR-JB9KRLDNMM-26KLBRRGZK',    						'Content-Type:application/json'    				),    		));    		    		$response = json_decode(curl_exec($curl), true);    		curl_close($curl);    		    		$this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|numeric|xss_clean');    		    		$this->form_validation->set_rules('product_variant_id', 'Product Variant Id', 'trim|required|xss_clean');    		    		$this->form_validation->set_rules('quantity', 'Quantities', 'trim|required|xss_clean');    		    		$this->form_validation->set_rules('promo_code', 'Promo Code', 'trim|xss_clean');    		    		$this->form_validation->set_rules('order_note', 'Special Note', 'trim|xss_clean');    		    		$this->form_validation->set_rules('latitude', 'Latitude', 'trim|numeric|xss_clean');    		    		$this->form_validation->set_rules('longitude', 'Longitude', 'trim|numeric|xss_clean');    		    		$this->form_validation->set_rules('delivery_date', 'Delivery Date', 'trim|xss_clean');    		    		$this->form_validation->set_rules('delivery_time', 'Delivery time', 'trim|xss_clean');    		    		$this->form_validation->set_rules('address_id', 'Address id', 'trim|required|numeric|xss_clean', array('required' => 'Please choose address'));    		    		    		    		$_POST['user_id'] = $this->data['user']->id;    		    		$_POST['is_wallet_used'] = 0;    		    		$data = array();    		    		if (!$this->form_validation->run()) {    		    			$this->response['error'] = true;    		    			$this->response['message'] = strip_tags(validation_errors());    		    			$this->response['data'] = array();    		    			print_r(json_encode($this->response));    		    			return;    		    		} else {    		    			$_POST['order_note'] = (isset($_POST['order_note']) && !empty($_POST['order_note'])) ? $this->input->post("order_note", true) : NULL;    		    		    		    			$product_variant_id = explode(',', $_POST['product_variant_id']);    		    			    		    			$quantity = explode(',', $_POST['quantity']);    		    			$check_current_stock_status = validate_stock($product_variant_id, $quantity);    		    			if ($check_current_stock_status['error'] == true) {    		    				$this->response['error'] = true;    		    				$this->response['message'] = $check_current_stock_status['message'];    		    				$this->response['data'] = array();    		    				print_r(json_encode($this->response));    		    				return false;    		    			}    		    		    		    			$cart = get_cart_total($_POST['user_id'], false, '0', $_POST['address_id']);    		    			if (empty($cart)) {    		    				$this->response['error'] = true;    		    				$this->response['message'] = "Your Cart is empty.";    		    				$this->response['data'] = array();    		    				print_r(json_encode($this->response));    		    				return false;    		    			}    		    			$_POST['delivery_charge'] = get_delivery_charge($_POST['address_id'], $cart['total_arr']);    		    			$_POST['delivery_charge'] = str_replace(',', '', $_POST['delivery_charge']);    		    			$_POST['is_delivery_charge_returnable'] = intval($_POST['delivery_charge']) != 0 ? 1 : 0;    		    			$wallet_balance = fetch_details('id=' . $_POST['user_id'], 'users', 'balance');    		    			$final_total = $cart['overall_amount'];    		    			$wallet_balance = $wallet_balance[0]['balance'];    		    			$_POST['wallet_balance_used'] = 0;    		    			if (isset($_POST['wallet_used']) && $_POST['wallet_used'] == 1) {    		    				if ($wallet_balance != 0) {    		    					$_POST['is_wallet_used'] = 1;    		    					if ($wallet_balance >= $final_total) {    		    						$_POST['wallet_balance_used'] = $final_total;    		    						$_POST['payment_method'] = 'wallet';    		    					} else {    		    						$_POST['wallet_balance_used'] = $wallet_balance;    		    					}    		    				} else {    		    					$this->response['error'] = true;    		    					$this->response['message'] = "Insufficient balance";    		    					$this->response['data'] = array();    		    					print_r(json_encode($this->response));    		    					return false;    		    				}    		    			}    		    			$promo_discount = 0;    		    			if (isset($_POST['promo_code']) && !empty($_POST['promo_code'])) {    		    				$validate = validate_promo_code($_POST['promo_code'], $this->data['user']->id, $cart['total_arr']);    		    				if ($validate['error']) {    		    					$this->response['error'] = true;    		    					$this->response['message'] = $validate['message'];    		    					print_r(json_encode($this->response));    		    					return false;    		    				} else {    		    					$promo_discount = $validate['data'][0]['final_discount'];    		    				}    		    			}    		    			$_POST['final_total'] = $cart['overall_amount'] - $_POST['wallet_balance_used'] - $promo_discount;    		    			if ($_POST['payment_method'] == "PAYTABS") {    		    				$_POST['active_status'] = "awaiting";    		    				$data['status'] = "success";    		    				$data['txn_id'] = null;    		    				$data['message'] = null;    		    			}     		    			$res = $this->order_model->place_order($_POST);    		    			$data['order_id'] = $res['order_id'];    		    			$data['user_id'] = $_POST['user_id'];    		    			$data['type'] = $_POST['payment_method'];    		    			$data['amount'] = $_POST['final_total'];    		    			if (($_POST['payment_method'] != "COD" && $_POST['payment_method'] != "Paypal") ||  $_POST['payment_method'] == "bank_transfer") {    		    				$this->transaction_model->add_transaction($data);    		    			}    		}    		    		    		$this->response['error'] = false;        		$this->response['message'] = 'trasaction initiated successfully';        		$this->response['data'] = $response;        		$this->response['data']['order_id'] = $order_id;        		print_r(json_encode($this->response));        		return;        	} else {        		$this->response['error'] = true;        		$this->response['message'] = "Unauthorised access is not allowed.";        		print_r(json_encode($this->response));        		return false;        	}        }
    ...
    }    public function paytabs_response($userId=null) {    	$this->load->model ( 'User_model', 'user');    	    	if ($this->ion_auth->logged_in()) {        		redirect('home', 'refresh');        	} else {    		    		$userDetails = $this->user->getSpecificUserDetail($userId, 1);    		     		$this->trigger_events('pre_set_session');    		     		$session_data = [    				     				'email'                    => $userDetails['email'],    				     				'user_id'                  => $userDetails['id'],    					    				'old_last_login'           => $userDetails['last_login'],    				     				'last_check'               => time(),    				     				'ion_auth_session_hash'    => $this->config->item('session_hash', 'ion_auth'),    					    				'identity'                    => $userDetails['mobile'],    					    				'mobile'                    => $userDetails['mobile'],    				     		];    		     		$this->session->set_userdata($session_data);    		     		$this->trigger_events('post_set_session');    		     		redirect('home', 'refresh');        	}        }
    public function trigger_events($events)        {        	if (is_array($events) && !empty($events))        	{        		foreach ($events as $event)        		{        			$this->trigger_events($event);        		}        	}        	else        	{        		if (isset($this->_ion_hooks->$events) && !empty($this->_ion_hooks->$events))        		{        			foreach ($this->_ion_hooks->$events as $name => $hook)        			{        				$this->_call_hook($events, $name);        			}        		}        	}        }
