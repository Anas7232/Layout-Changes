...
class Category_model extends CI_Model
{
    ...
    }        public function fetchAllSubCatgories($category_id) {        	$this->db->select ( "a.*", false );    	$this->db->from ( "categories a" );        	$this->db->where ( 'a.parent_id', $category_id );    	        	$result = $this->db->get ();    	    	$type = array ();$i=0;    	foreach ( $result->result () as $row ) {        		$type [$i]['id'] = $row->id;    		$type [$i]['name'] = $row->name;    		$type [$i]['parent_id'] = $row->parent_id;    		$type [$i]['slug'] = $row->slug;    		$type [$i]['image'] = $row->image;    		$type [$i]['status'] = $row->status;    		    		$i++;    	}    	return $type;    }
}
