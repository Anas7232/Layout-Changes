...
//        $('#place_order_btn').attr('disabled', true).html('Please Wait...');
        ...
        var payment_methods = $("input[name='payment_method']:checked").val();                console.log(payment_methods);
        ...
        } else if (payment_methods == "PAYTABS") {        	place_order_paytabs().done(function (result) {                if (result.error == false) {                	                	                	window.location.replace(result.data.redirect_url);                	                                    } else {                    Toast.fire({                        icon: 'error',                        title: result.message                    });                }            });        }else if (payment_methods == "Flutterwave") {
            flutterwave_payment();
        } else if (payment_methods == "COD" || payment_methods == "Direct Bank Transfer" ) {
            place_order().done(function (result) {
                if (result.error == false) {                	                	                	window.location.reload();                	
    ...
    }        function place_order_paytabs() {        let myForm = document.getElementById('checkout_form');        var formdata = new FormData(myForm);        formdata.append(csrfName, csrfHash);        formdata.append('promo_code', $('#promocode_input').val());        formdata.append('user_id', $("#user_id").val());                var latitude = sessionStorage.getItem("latitude") === null ? '' : sessionStorage.getItem("latitude");        var longitude = sessionStorage.getItem("longitude") === null ? '' : sessionStorage.getItem("longitude");        formdata.append('latitude', latitude);        formdata.append('longitude', longitude);        return $.ajax({            type: 'POST',            data: formdata,            url: base_url + 'payment/initiate-paytabs-transaction',            dataType: 'json',            cache: false,            processData: false,            contentType: false,            beforeSend: function () {                $('#place_order_btn').attr('disabled', true).html('Please Wait...');            },            success: function (data) {            	console.log(data);            	return;            	                csrfName = data.csrfName;                csrfHash = data.csrfHash;                $('#place_order_btn').attr('disabled', false).html('Place Order');                if (data.error == false) {                    Toast.fire({                        icon: 'success',                        title: data.message                    });                } else {                    Toast.fire({                        icon: 'error',                        title: data.message                    });                }            }        })    }
